...
$selectStu=ejecutarSQL::consultar("SELECT * FROM estudiante WHERE DNI='".$primaryKey."'");
...
$selectAllLoansS=ejecutarSQL::consultar("SELECT * FROM prestamoestudiante WHERE DNI='".$primaryKey."'");
$totalP=0;
...
if($totalP<=0){
    $totalErrors=0;
    $selectAllLoansStud=ejecutarSQL::consultar("SELECT * FROM prestamoestudiante WHERE DNI='".$primaryKey."'");
    ...
    if($totalErrors<=0){
            if(consultasSQL::DeleteSQL("bitacora", "CodigoUsuario='".$primaryKey."' AND Tipo='Estudiante'")){
                if(consultasSQL::DeleteSQL("estudiante", "DNI='".$primaryKey."'")){
                    if($NumRep>1){
